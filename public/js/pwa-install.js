...
class PWAInstaller {
  ...
  showNotification(message, type = 'info') {
    ...
    notification.innerHTML = `
      <div style="display: flex; align-items: center; gap: 10px; padding: 16px 20px; margin: 0;">
        <i class="fas ${this.getIconForType(type)}"></i>
        <span>${message}</span>
        <button onclick="this.parentElement.parentElement.remove()" style="background: none; border: none; color: #999; cursor: pointer; margin-left: auto; padding: 0;">
    ...
    // إخفاء الإشعار بعد 5 ثوان (قابل للإغلاق يدوياً)
    setTimeout(() => {
  ...
  showUpdateNotification() {
    ...
    notification.innerHTML = `
      <div style="display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; margin: 0;">
        ...
        <div style="display: flex; gap: 8px;">
          ...
          <button onclick="this.parentElement.parentElement.parentElement.remove()" style="background: none; border: none; color: #999; cursor: pointer; padding: 0;">
            <i class="fas fa-times"></i>
          </button>
        </div>
      </div>
  ...
  enablePushNotifications() {
    if ('Notification' in window && 'serviceWorker' in navigator) {
        ...
        if (permission === 'granted') {
          ...
          setTimeout(() => {
            new Notification('مرحباً بك!', {
              body: 'تم تفعيل الإشعارات بنجاح في موقع اعملها براك',
              icon: '/icons/icon-192x192-v4.png',
              badge: '/icons/icon-72x72-v4.png',
      ...
      this.showNotification('الإشعارات غير مدعومة في هذا المتصفح', 'warning');
    }
  }
